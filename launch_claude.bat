@echo off
REM DevEnviro + Claude Code Integrated Launcher (Windows)
REM Flexible launcher with multiple options

setlocal EnableDelayedExpansion

echo DevEnviro + Claude Code Integrated Launcher
echo ==========================================

REM Change to script directory
cd /d "%~dp0"

REM Parse command line arguments
set DEVENVIRO_ONLY=0
set CLAUDE_ONLY=0
set SKIP_INTERACTIVE=0
set PROJECT_PATH=
set SHOW_HELP=0

:parse_args
if "%~1"=="" goto end_parse
if "%~1"=="--devenviro-only" (
    set DEVENVIRO_ONLY=1
    shift
    goto parse_args
)
if "%~1"=="--claude-only" (
    set CLAUDE_ONLY=1
    shift
    goto parse_args
)
if "%~1"=="--skip-interactive" (
    set SKIP_INTERACTIVE=1
    shift
    goto parse_args
)
if "%~1"=="--project-path" (
    set PROJECT_PATH=%~2
    shift
    shift
    goto parse_args
)
if "%~1"=="--help" (
    set SHOW_HELP=1
    shift
    goto parse_args
)
if "%~1"=="-h" (
    set SHOW_HELP=1
    shift
    goto parse_args
)
REM Unknown argument, skip it
shift
goto parse_args

:end_parse

REM Show help if requested
if %SHOW_HELP%==1 (
    echo.
    echo Usage: launch_claude.bat [OPTIONS]
    echo.
    echo Options:
    echo   --devenviro-only     Run DevEnviro startup only ^(no Claude Code^)
    echo   --claude-only        Launch Claude Code only ^(skip DevEnviro^)
    echo   --skip-interactive   Skip DevEnviro interactive menu
    echo   --project-path PATH  Launch in specific project directory
    echo   --help, -h           Show this help message
    echo.
    echo Examples:
    echo   launch_claude.bat                          # Full integrated launch
    echo   launch_claude.bat --devenviro-only         # DevEnviro only
    echo   launch_claude.bat --claude-only            # Claude Code only
    echo   launch_claude.bat --skip-interactive       # Non-interactive mode
    echo.
    goto end
)

REM Handle project path change
if not "%PROJECT_PATH%"=="" (
    echo [INFO] Switching to project: %PROJECT_PATH%
    if exist "%PROJECT_PATH%" (
        cd /d "%PROJECT_PATH%"
    ) else (
        echo [ERROR] Project path does not exist: %PROJECT_PATH%
        goto error_exit
    )
)

REM Step 1: Run DevEnviro startup (unless claude-only)
if %CLAUDE_ONLY%==0 (
    echo.
    echo [LAUNCHER] Starting DevEnviro initialization...
    
    if %SKIP_INTERACTIVE%==1 (
        echo [INFO] Running DevEnviro in non-interactive mode
        echo 7 | python devenviro_startup.py
    ) else (
        python devenviro_startup.py
    )
    
    if %ERRORLEVEL% neq 0 (
        echo [WARNING] DevEnviro startup failed, continuing anyway...
    ) else (
        echo [SUCCESS] DevEnviro startup completed
    )
)

REM Step 2: Launch Claude Code (unless devenviro-only)
if %DEVENVIRO_ONLY%==0 (
    echo.
    echo [LAUNCHER] Starting Claude Code...
    
    REM Try claude-code command first
    where claude-code >nul 2>&1
    if %ERRORLEVEL%==0 (
        echo [INFO] Launching Claude Code...
        start "" claude-code .
        echo [SUCCESS] Claude Code launched
    ) else (
        REM Fallback to VS Code
        where code >nul 2>&1
        if %ERRORLEVEL%==0 (
            echo [INFO] Claude Code not found, launching VS Code as fallback...
            start "" code .
            echo [SUCCESS] VS Code launched as fallback
        ) else (
            echo [ERROR] Neither 'claude-code' nor 'code' commands found
            echo [INFO] Please install Claude Code CLI or VS Code
            goto error_exit
        )
    )
)

echo.
echo [SUCCESS] Launch sequence completed!
if %DEVENVIRO_ONLY%==1 (
    echo DevEnviro workspace is ready for development.
) else if %CLAUDE_ONLY%==1 (
    echo Claude Code is starting...
) else (
    echo DevEnviro workspace is initialized and Claude Code is starting...
)

goto end

:error_exit
echo.
echo [ERROR] Launch sequence encountered issues
pause
exit /b 1

:end
pause
exit /b 0